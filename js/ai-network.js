...
if(!paused&&!reduced&&!dormant){let t=-1;if(intro<1){if(a<3&&intro>.18&&intro<.52)t=(intro-.18)/.34;else if(b===17&&intro>.55&&intro<.85)t=(intro-.55)/.3;else if(b===21&&intro>.85)t=(intro-.85)/.15}else if(time%16000<2600&&[20,25].includes(i))t=(time%16000)/2600;if(t>=0&&t<=1){const p=project(curvePoint(g,t));primitives.push({z:p.z+.01,draw:()=>{ctx.beginPath();ctx.arc(p.x,p.y,2.4,0,Math.PI*2);ctx.fillStyle='#b88c3d';ctx.fill()}})}}});
