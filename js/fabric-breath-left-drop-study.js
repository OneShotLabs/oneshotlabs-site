...
(()=>{'use strict';
const canvas=document.querySelector('#fabric-breath'),section=canvas.closest('section'),button=document.querySelector('#motion-toggle');
let gl,program,time=0,last=0,raf=0,visible=true,paused=false,light=0,target=0,fall=0,fallTarget=0;
const running=()=>!paused;
const wake=()=>{if(!raf&&visible&&!document.hidden&&gl)raf=requestAnimationFrame(draw)};
function label(){button.hidden=false;button.textContent=running()?'Pause motion':'Play motion';button.setAttribute('aria-pressed',String(!running()));section.dataset.motion=running()?'playing':'paused'}
function draw(now){raf=0;const dt=Math.min((now-(last||now))/1000,.05);last=now;if(running()){time+=dt;light+=(target-light)*(1-Math.exp(-dt*1.4));fall+=(fallTarget-fall)*(1-Math.exp(-dt*1.65))}gl.uniform1f(gl.getUniformLocation(program,'t'),time);gl.uniform1f(gl.getUniformLocation(program,'light'),light);gl.uniform1f(gl.getUniformLocation(program,'drop'),fall);gl.drawArrays(gl.TRIANGLES,0,6);canvas.dataset.time=time.toFixed(3);canvas.dataset.drop=fall.toFixed(3);if(running())wake()}
button.addEventListener('click',()=>{paused=!paused;label();last=0;wake()});
...
function syncFall(){fallTarget=document.body.classList.contains('exploring-system')?1:0;last=0;wake()}
new MutationObserver(syncFall).observe(document.body,{attributes:true,attributeFilter:['class']});syncFall();
...
const fs=`precision highp float;varying vec2 uv;uniform sampler2D plate;uniform float t;uniform float light;uniform float drop;
...
// The upper-right suspension point stays pinned while the left span yields to gravity.
float leftSpan=1.-smoothstep(.08,.63,p.x);
float hanging=smoothstep(.01,.72,p.y);
float settledWave=.012*sin(t*.29+p.x*3.1+1.2)*leftSpan*drop;
dy+=(-.145*leftSpan*(.34+.66*hanging)*drop)+settledWave;
dx+=.012*leftSpan*(p.y-.18)*drop;
