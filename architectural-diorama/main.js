...
document.querySelector('#app').innerHTML = `
  <section class="hero" data-phase="inputs" aria-label="OneShotLabs turns fragmented real estate evidence into investment judgment">
    <div class="production" aria-hidden="true">
      <video class="diorama-film" muted playsinline preload="auto">
        <source src="OneShotLabs-Diorama-Master-16s.mp4" type="video/mp4">
      </video>
      <div class="seal-surface"></div>
    ...
    <div class="editorial">
      ...
      <p class="frontier-tagline">Real Estate Intelligence for the Frontier</p>
...
function paint(t) {
  ...
  const compression = Math.max(0, Math.min(1, (t - 8) / (filmHold - 8)));
  const compressionEase = compression * compression * (3 - 2 * compression);
  hero.style.setProperty('--production-scale', String(.62 * (1 - .12 * compressionEase)));
  const blankIn = Math.max(0, Math.min(1, (t - 14.5) / 0.7));
  const blankOut = Math.max(0, Math.min(1, (t - 17.65) / 0.25));
  const sealIn = Math.max(0, Math.min(1, (t - 15.45) / 0.9));
  const sealOut = Math.max(0, Math.min(1, (t - 16.85) / 0.8));
...
function frame(now) {
  let t = paused ? pausedAt : 0;
  if (!paused && film.readyState >= 2) {
    if (film.currentTime < filmHold - .025) {
      extensionStart = null;
      t = film.currentTime;
    } else {
      ...
      t = filmHold + (now - extensionStart) / 1000;
      if (t >= duration) {
        t = 0;
        extensionStart = null;
        film.currentTime = 0;
...
function setPaused(value) {
  ...
  if (value) pausedAt = currentT;
  paused = value;
  if (value) film.pause();
  else {
    film.currentTime = Math.min(pausedAt, filmHold);
    if (pausedAt < filmHold) {
      extensionStart = null;
      film.play().catch(() => {});
    } else {
      extensionStart = performance.now() - (pausedAt - filmHold) * 1000;
    }
  }
...
document.querySelectorAll('[data-jump]').forEach((button) => button.addEventListener('click', () => {
  ...
  film.currentTime = Math.min(pausedAt, filmHold);
  extensionStart = pausedAt >= filmHold ? performance.now() - (pausedAt - filmHold) * 1000 : null;
  currentT = pausedAt;
  paint(pausedAt);
}));
